...
digits<-read_excel(here("raw","GEOCodigo.xls"))%>%select(1:3)%>%
  rename(municipality_name=MUNICÍPIO)
...
transfer_conv<-siconfi_receipts%>%filter(str_detect(conta_bd,"^Transferência de Convênios$"))%>%arrange(id_municipio,ano, portaria)

#Before 2013, deductions are registered under "Deduções da Receita Corrente", then afterwards they are dealt with at the item-level with a separate category under "estagio"
...
siconfi_receipts <- siconfi_receipts %>%
  mutate(estagio_bd = if_else(conta_bd == "Deduções da Receita Corrente", "Outras Deduções da Receita", estagio_bd),
         conta_bd = if_else(conta_bd == "Deduções da Receita Corrente", "Receitas Correntes", conta_bd))
...
panel$nome_municipio[panel$id_municipio==1504752]<-"Mojuí dos Campos"
panel$nome_municipio[panel$id_municipio==4212650]<-"Pescaria Brava"
panel$nome_municipio[panel$id_municipio==4220000]<-"Balneário Rincão"
panel$nome_municipio[panel$id_municipio==4314548]<-"Pinto Bandeira"
panel$nome_municipio[panel$id_municipio==5006275]<-"Paraíso das Águas"
